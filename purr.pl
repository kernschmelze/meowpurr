...
sub meow_f_diropenwith {
  ...
  if (scalar @mrufms) {
    foreach my $appx (@mrufms) {
      ...
      $os .= "+ \t\"" . $appn . "\"\tExec exec $meowhelper --kludge 'mode=$mru_p_mode_f_fileopen;app=" . substr($appx,2) . ";file=$dir'\n";
    }
    $os .= "+ \"\" Nop\n" if (scalar @othfms);
  }
  foreach (sort keys %ni) {
    $os .= "+ \t\"$_\"\tExec exec $meowhelper --kludge 'mode=$mru_p_mode_f_fileopen;app=" . substr($ni{$_},2) . ";file=$dir'\n";
  }
  prout('Y');
}
...
sub meow_f_fileopenwith {
  ...
  if (exists $mru {'mru_lastapps'}) {
	...
	if (exists ${$list_apps_fields { $app }} {'mimefexts'}) {
	  ...
	  if ($fex == 1) {
	    ...
	    $os .= ' + "' . ${$list_apps_fields { $app }} {'name'} .
		  "\"\tExec exec $meowhelper --kludge 'mode=$mru_p_mode_f_fileopen;app=$app;file=$pat'\n";
	    # store it
	    push (@appslisted, $app);
  } } } } }
  ...
  if (scalar @matchapps > 0) {
    ...
    foreach my $appt (sort keys %ni) {
      $os .= "+ \t\"$appt\"\tExec exec $meowhelper --kludge 'mode=$mru_p_mode_f_fileopen;app=$ni{$appt};file=$pat'\n";
  } }
  prout('W');
}
...
sub getexecstr {
  ...
  my $action = shift;
  my $path = shift;
  my $execstr = "Exec exec $meowhelper ";
